              ...
              <TableCell>
                <div className="space-y-1">
                  ...
                  {application?.cousrseEnrolled?.[application.cousrseEnrolled?.length - 1]?.tokenFeeDetails?.verificationStatus === 'paid' ?
                    <div className="text-xs font-normal">₹ {formatAmount(application?.cohort?.cohortFeesDetail?.tokenFee)}</div> :
                    <div className="text-xs text-muted-foreground">Interview cleared {timeAgo(application?.applicationDetails?.updatedAt)}</div>
                  }
                </div>
              </TableCell>
              <TableCell>
