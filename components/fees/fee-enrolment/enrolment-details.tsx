        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant={getStatusColor(application?.cousrseEnrolled?.[application.cousrseEnrolled?.length - 1]?.tokenFeeDetails?.verificationStatus || "")} className="capitalize">
                {application?.cousrseEnrolled?.[application.cousrseEnrolled?.length - 1]?.tokenFeeDetails?.verificationStatus}
              </Badge>
            </div>
            <div className="flex justify-between items-center text-xs">
              ...
              {application?.cousrseEnrolled?.[application.cousrseEnrolled?.length - 1]?.tokenFeeDetails?.verificationStatus === 'paid' ? 
                <p className="text-muted-foreground">Admission Fee: <span className="text-xs text-white font-normal">₹ {formatAmount(application?.cohort?.cohortFeesDetail?.tokenFee)}</span></p> :
                <p className="text-muted-foreground">Interview Cleared {timeAgo(application?.applicationDetails?.updatedAt)}</p>
              }
            </div>
          </div>
