...
type BadgeVariant = "pending" | "warning" | "secondary" | "success" | "onhold" | "default";
...
export function PaymentInformationTab({ student, onApplicationUpdate }: PaymentInformationTabProps) {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setReceiptFile(file);
      setReceiptPreviewUrl(URL.createObjectURL(file)); // Create a preview URL for the image
    }
  };
  ...
  const handleUploadReceipt = async () => {
    ...
    try {
      // Here you can add your file upload logic, for example, uploading to an API or server
      console.log("Uploading file", receiptFile);

      // Clear the selected file and preview URL after uploading
      setReceiptFile(null);
